process VARSCAN_SOMATIC {
    ...
    tuple val("${task.process}"), val('varscan'), eval('varscan 2>&1 | grep VarScan | head -n 1 | sed "s/VarScan //g" | sed "s/,.*//g" | sed "s/^v//" | sed "s/_.*//"'), emit: versions_varscan, topic: versions
